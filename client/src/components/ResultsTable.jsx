export default function ResultsTable({ result }) {
  ...
  <details>
    <summary>View runtime violations</summary>
    <ul>
      {page.violations.map((v, idx) => (
        <li key={idx}>
          <strong>{v.id}</strong> ({v.impact}) — {v.description}{" "}
          <a href={v.helpUrl} target="_blank" rel="noreferrer">Help</a>

          {v.nodes?.length > 0 && (
            <ul style={{ marginTop: 4 }}>
              {v.nodes.map((n, nIdx) => (
                <li key={nIdx}>
                  Target: {n.target?.join(", ")} <br />
                  Line: {n.location?.line ?? "N/A"}, Column: {n.location?.column ?? "N/A"} <br />
